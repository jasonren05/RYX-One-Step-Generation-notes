...
- [Applications](#applications)
  ...
  - [One-Step Generation + GAN in Video Generation](#one-step-generation--gan-in-video-generation)
...
Flow Matching / Rectified Flow 都可以从任意 coupling 构造速度目标。最初的 Rectified Flow 就是在随机 coupling 上训练速度场。Reflow 则利用已经训练好的 ODE 重新生成 deterministic coupling (被蒸馏网络产生的噪声-图片对​)，再用 deterministic couple 训练新的速度场，使轨迹变直。最后 Distillation 才在已经直化的 flow 上学习一步映射，从而减少采样步骤。
...
> **Core principle — push samples onto the data manifold with adversarial correction.** Borrowing the GAN principle, these methods use either an external discriminator or an endogenous real/fake score discrepancy to drive generated samples toward the data distribution, yielding sharp, high-fidelity outputs in a single step. Often combined with score-, flow-, or consistency-based objectives.
...
> [!NOTE]
> 把普通 Flow Matching 的时间范围从 [0,1] 扩展到 [-1,1]：正时间分支学习噪声到真实数据的分布，负时间分支学习噪声到模型自己生成的 fake 数据的分布。  
> 同一个网络用时间正负区分 real/fake 速度场（用t正负作为一个identifier，告诉模型要学习真实数据/假分布），两个速度的方向差就是“自对抗”信号，用它把真实轨迹拉直。以此去掉了冻结的老师、额外的 fake-score 网络或 GAN 判别器（DMD2中这几个都用到了），扩展性更好，可以即插即用。

- **Self-Adversarial One Step Generation via Condition Shifting (APEX)** [2026] 🟢  
[[Paper](https://arxiv.org/abs/2604.12322)] [[Code](https://github.com/LINs-lab/APEX)]  
Discriminator-free self-adversarial flow training that separates real and fake velocity estimators by an affine shift in condition space; architecture-preserving and compatible with full-parameter or LoRA tuning.

> [!NOTE]
> APEX用guidance作为网络学习内容的标记（c取真实值的时候，学习真实图像的分布；c取c_fake的时候，学习学生模型当前学习到的分布。一个模型当两个用，形成自对抗），具体来说，把文本条件从 c 变换为 c_fake = Ac + b。真实条件分支学数据速度场，偏移条件分支跟踪模型当前的 fake 分布，两者的速度差就提供自对抗修正（问题：速度差是如何在训练目标里体现的？）。  
> 它不新增判别器，而是让同一个 Flow 模型借助两种条件承担“生成器 + fake-score 估计器”两个角色。其梯度可写成GAN的score difference形式。
...
### One-Step Generation + GAN in Video Generation

- **Self Forcing: Bridging the Train-Test Gap in Autoregressive Video Diffusion** [NeurIPS 2025 Spotlight] 🔵  
[[Paper](https://arxiv.org/abs/2506.08009)] [[Code](https://github.com/guandeh17/Self-Forcing)] [[Project](https://self-forcing.github.io/)]  
...
| 2026.06 | AAD-1: Asymmetric Adversarial Distillation for One-Step Autoregressive Video Generation        | ICML 2026              | 🔵   | APP      | [[Paper](https://arxiv.org/abs/2606.03972)] [[Code](https://github.com/AutoLab-SAI-SJTU/AAD-1)] [[Project](https://aad-1.github.io/)]                                                                         |
...
| 2026.04 | Self-Adversarial One Step Generation via Condition Shifting (APEX)                             | 2026                   | 🟢   | ADV      | [[Paper](https://arxiv.org/abs/2604.12322)] [[Code](https://github.com/LINs-lab/APEX)]                                                                                                                        |
...
| 2025.06 | Self Forcing: Bridging the Train-Test Gap in Autoregressive Video Diffusion                    | NeurIPS 2025 Spotlight | 🔵   | APP      | [[Paper](https://arxiv.org/abs/2506.08009)] [[Code](https://github.com/guandeh17/Self-Forcing)] [[Project](https://self-forcing.github.io/)]                                                                  |
